...
class BLESpamScreen(Screen):
        ...
        try:
            ...
            while not self._stop_event.is_set():
                ...
                if profile_data == b"ALL":
                    actual = [p for p in PROFILES if p[1] != b"ALL"]
                    profile_data = random.choice(actual)[1]
    ...
    def draw(self, surf, theme):
        ...
        self.app.statusbar.draw(surf, theme, "BT: BLE SPAM")
        ...
        status_text = f"SPAMMING: {self.packets_sent}" if self.running else "STATUS: READY"
        ...
        surf.blit(small.render(f"ADAPTER: {self.iface}", True, theme.color("text_dim")), (status_rect.x + 15, status_rect.y + 35))
        ...
        for i, (name, _) in enumerate(PROFILES):
            ...
            surf.blit(small.render(name, True, p_color), (20, y))
